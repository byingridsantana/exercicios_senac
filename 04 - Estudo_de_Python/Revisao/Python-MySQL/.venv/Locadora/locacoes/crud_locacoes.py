...
# Função para cadastrar os dados da locacoes
...
def atualizar_locacoes(data_inicio, data_fim, valor_total, id_veiculo, id_cliente, forma_pagamento):
    banco, cursor = conectar_banco()
    sql = "UPDATE locacoes SET data_inicio=%s, data_fim=%s, valor_total=%s, id_veiculo=%s, id_cliente=%s, forma_pagamento=%s WHERE id_funcionario=%s" 
    val = (data_inicio, data_fim, valor_total, id_veiculo, id_cliente, forma_pagamento, id)
    cursor.execute(sql,val)
    banco.commit()
    cursor.close()
    banco.close()
    print("Os dados da locação foram atualizados!")
...
def apagar_locacoes(id):
    ...
    cursor.close()
    banco.close()
    print("Os dados da locação foram apagados!")